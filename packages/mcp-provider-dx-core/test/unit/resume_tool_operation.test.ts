...
import { ResumeMcpTool } from '../../src/tools/resume_tool_operation.js';

/**
 * T06 unit tests for resume_tool_operation:
...
describe('resume_tool_operation unit', () => {
  const sandbox = sinon.createSandbox();
  afterEach(() => sandbox.restore());

  const allowedOrgs = ['OMNI_Staging', 'OMNI_Admin'];
  // A valid deploy job ID (prefix 0Af)
  const validDeployJobId = '0Af000000000001AAA';
  ...
  it('Test A: empty usernameOrAlias returns error listing allowed orgs', async () => {
    ...
    const tool = new ResumeMcpTool(services);

    const result = await tool.exec({ jobId: validDeployJobId, wait: 1, usernameOrAlias: '', directory: '/tmp' });
  ...
  it('Test B: omitting directory does not cause directory-related failure', async () => {
    ...
    // This will attempt to resume and likely fail on the actual deploy polling,
    // but must NOT fail because directory is undefined
    const result = await tool.exec({
      jobId: validDeployJobId,
      wait: 1,
